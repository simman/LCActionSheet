...
- (void)didClickBtn:(UIButton *)btn {
    ...
    if (SDefauldCustomizationBlock) {
        SDefauldCustomizationBlock(btn.tag, self);
        return;
    }
...
- (void)didClickCancelBtn {
    ...
    [UIView animateWithDuration:0.3f
                     ...
                     completion:^(BOOL finished) {
                         ...
                         if (SDefauldCustomizationBlock) {
                             SDefauldCustomizationBlock(_buttonTitles.count, self);
                             return;
                         }
...
+ (id)showSheetWithTitle:(NSString *)title
            buttonTitles:(NSArray *)titles
          redButtonIndex:(NSInteger)buttonIndex
         completionBlock:(void (^)(NSUInteger, LCActionSheet *))block
{
    LCActionSheet *actionSheet = [[LCActionSheet alloc] initWithTitle:title buttonTitles:titles redButtonIndex:buttonIndex block:^(NSUInteger buttonIndex, SMActionSheet *actionSheet) {
        if (block) {
...
- (instancetype)initWithTitle:(NSString *)title
                 buttonTitles:(NSArray *)titles
               redButtonIndex:(NSInteger)buttonIndex
                     block:(SCustomizationBlock)block {
    
    if (self = [super init]) {
        
        SDefauldCustomizationBlock = block;
